...
- The quoted thread title is the root message's text (the event whose id equals the root `e` tag or `reply_to`), looked up in this record or in catch-up *event* entries. If that id is not present, the header prints `(thread)` with no title — never the first catch-up line.
