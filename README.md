...
2. **Resolves the event behind each prompt.** From `_meta.buzz.events[]` when the harness attaches it (the structured shape in flight upstream; the last event routes), else from the harness's text framing — the `<buzz-event>` block with its `Event ID:`, `Channel:`, `Kind:`, `From: … (hex: …)`, `Time:`, `Content:`, `Tags:` lines (the last block of a `<buzz-events>` batch) — else a synthetic event: a stable sha256 id, sender unknown, the raw prompt as content. The prompt itself always travels whole.
...
Not applied here, because the harness already did it: the respond-to gate, deduplication, session scope, thread context. Not needed here: a relay URL, a key, a state file beyond the delivery log.
