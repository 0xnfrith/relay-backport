...
- **`claude-code` view.** For each record, two lines in **one write**: a `WAKE` header (type, channel name, scope, thread title, purpose, sender, class, owner, reply anchor, channel uuid, short event id, time, catch-up count by speaker, content length) and a `TEXT` line (short id + message, newlines shown as ` ⏎ `). Lifecycle `EVENT|` lines pass through. The visible budget is `view.visible_chars` (default 500); the writer enforces every cap so the header never exceeds it. Truncate, never wrap. A message body that starts with `WAKE ` or `TEXT |` stays on the TEXT line and cannot forge a header.
